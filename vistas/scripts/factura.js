        ...
        url: '../ajax/factura.php?op=listarArticulosfactura&tipoprecioaa='+tipoprecio+'&tipof='+tpf+'&itm='+$iteno+'&alm='+almacen,
                ...
                "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
                    ...
                    $(nRow).find('td:eq(5)').html(
                        '<div class="row">' +
                            '<div class="col-8">' +
                                '<input type="number" class="form-control" style="width: 100px;" id="product_stock" value="' + aData[5] + '" />' +
                            '</div>'+
                            '<div class="col-2 p-0">' +
          ...
          if (result.value) {
            ...
            $.ajax({
                ...
                success: function(datos)
                {
                      ...
                      //listar();
                      refrescartabla();
                      //window.location="https://yougrowss.servitecfl.com/vistas/factura.php";
    ...
    if (result.value) {
      $.post("../ajax/factura.php?op=generarxml", { idfactura: idfactura }, function (e) {
        ...
        Swal.fire({
          ...
          html: 'Se ha generardo el archivo XML: <a href="'+data.cabextxml+'" download="'+data.cabxml+'">'+data.cabxml+'</a>, haga clic en el nombre para descargarlo.',
          icon: 'success',
          timer: 2000,
          showConfirmButton: false
        });
...
function baja(idfactura) {
  ...
  Swal.fire({
    ...
    if (result.value) {
      $.post("../ajax/factura.php?op=baja&comentario=" + result.value + "&hora=" + cad, { idfactura: idfactura }, function (e) {
        Swal.fire({
          title: '¡Proceso completado!',
          text: e,
          icon: 'success',
...
function agregarDetalle(tipoagregacion ,idarticulo, familia, codigo_proveedor, codigo, nombre,
    ...
    cicbper, mticbperuSunat, factorconversion, factorc, nombreum, descrip, tipoitem)
  {
    var cantidad=0;
     if (idarticulo!="")
    {
        ...
        '<td><input type="text" class="form-control" name="stock[]" id="stock[]" value="'+factorconversion+'" disabled="true" size="4" ></td>'+
  ...
  function agregarArticuloxCodigo(e)
  {
    ...
    $.post("../ajax/factura.php?op=listarArticulosfacturaxcodigo&codigob="+codigob+"&idempresa="+$idempr+"&tipp="+tipopre, function(data,status)
    {
      ...
      var fila='<tr class="filas" id="fila'+cont+'">'+
        ...
        '<td><span name="numero_orden" id="numero_orden'+cont+'"></span>'+
        '<input type="hidden" name="numero_orden_item[]" id="numero_orden_item[]" value="'+conNO+'" size="1" ></td>'+
        '<td><input type="hidden" name="idarticulo[]" style="font-family: times, serif; font-size:10pt; font-style:italic" value="'+data.idarticulo+'"> <input type="text" class="form-control" style="display:none;" name="descdet_[]" id="descdet_[]" onkeyup="mayus(this)" onkeypress="return focusDescdet(event, this)">'+data.nombre+'</td>'+
        ...
        '<td><input type="text" class="form-control" name="valor_unitario[]" id="valor_unitario[]" value="'+data.precio_venta+'"   '+ rd +' onBlur="modificarSubototales(1)" size="5" onkeypress="return NumCheck2(event, this)" font-weight:bold;" OnFocus="focusTest(this);"></td>'+
        '<td><input type="text" class="form-control" name="valor_unitario2[]" id="valor_unitario2[]" size="5"  value="'+precioOculto+'"    '+ exo +' onBlur="modificarSubototales(1)" OnFocus="focusTest(this);"></td>'+
        '<td><input type="text" class="form-control" name="stock[]" id="stock[]" value="'+data.factorconversion+'" disabled="true" size="4" ></td>'+
        '<td><span name="subtotal" id="subtotal'+cont+'"></span>'+
        '<input type="hidden" name="subtotalBD[]" id="subtotalBD["'+cont+'"]">'+
        '<span name="igvG" id="igvG'+cont+'" style="background-color:#9fde90bf;display:none;"></span>'+
...
function agregarClientexRuc(e) {
    ...
    if (e.keyCode === 13 && !e.shiftKey) {
      $.post("../ajax/factura.php?op=listarClientesfacturaxDoc&doc="+documento, function(data,status) {
        data = JSON.parse(data);
        if (data != null && data.idpersona != null) { // Agregamos verificación adicional para la entrada nula
          $('#idpersona').val(data.idpersona);
          $("#razon_social2").val(data.razon_social);
          $('#domicilio_fiscal2').val(data.domicilio_fiscal);
          $('#correocli').val(data.email);
...
function agregarItemdetalle()
  {
    ...
    if ($("#nombre_tributo_4_p").val()=='9997')
        ...
        '<td><span name="numero_orden" id="numero_orden'+cont+'"></span>'+
        '<input type="hidden" name="numero_orden_item[]" id="numero_orden_item[]" value="'+conNO+'" size="1" ></td>'+
        '<td><input type="hidden" name="idarticulo[]" style="font-family: times, serif; font-size:10pt; font-style:italic" value="'+$idarticulo+'"> <input type="text" class="form-control" style="display:none;" name="descdet_[]" id="descdet_[]" onkeyup="mayus(this)" onkeypress="return focusDescdet(event, this)">'+$nombre+'</td>'+
        ...
        '<td><input type="text"  required="true" class="form-control" name="cantidad[]" id="cantidad[]"   size="2" onkeypress="return NumCheck(event, this)" value="'+$cantidad+'"></td>'+
        ...
        '<td><input type="text" class="form-control" name="valor_unitario[]" id="valor_unitario[]" value="'+$precio_factura+'"   '+ rd +'  size="5" onkeypress="return NumCheck2(event, this)"  OnFocus="focusTest(this);"  style="font-size:12px;"></td>'+
        '<td><input type="text" class="form-control" name="valor_unitario2[]" id="valor_unitario2[]" size="5"  value="'+precioOculto+'"    '+ exo +' ></td>'+
