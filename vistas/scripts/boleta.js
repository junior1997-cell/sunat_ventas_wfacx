...
function limpiar() {
   ...
   $("#ccuotas").val("0");
    ...
    inputFechaVenc.addEventListener("change", function() {
      ...
      const diasDiferencia = Math.round((fechaSeleccionada - fechaActual) / (1000 * 60 * 60 * 24));
...
//$(tabla.container()).find("div.tbllistado_filter input").focus();
        ...
        "ajax":
                ...
                "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
                    ...
                    $(nRow).find('td:eq(5)').html(
                        '<div class="row">' +
                            '<div class="col-8">' +
                                '<input type="number" class="form-control hidebutton text-end" style="width: 100px;" id="product_stock" value="' + aData[5] + '" />' +
                            '</div>'+
                            '<div class="col-2 p-0">' +
                    ...
                    if ( aData[5] == "0.00" )
                    {
                        ...
                        $(nRow).find('#btn_editarstock').click(function() {
...
function mostrarModal(data) {
    ...
    Swal.fire({
        ...
        html:
            '<img class="mb-3" style="width:80%;" src="' + img_product +  '" />'+
...
$("#ipagado").on("input", function() {
  ...
  var importe_pagado = parseFloat( $(this).val() );
  var total_pagar = parseFloat( $('#total').text() );
  ...
  if ( importe_pagado > total_pagar ) {
    ...
    $('#vuelto_text').html( 'Vuelto :  <span style="color: red; font-weight: bold">Falta</span>' );
    ...
    tipoitem)
  {
     ...
     if (idarticulo!="")
    {
       ...
       // <textarea class="" name="descdet[]" id="descdet[]" rows="1" cols="70" onkeyup="mayus(this)" onkeypress="return focusDescdet(event, this)">'+descrip+'</textarea>
        ...
        '<td hidden><select name="codigotributo[]" class="" style="display:none;"> <option value="1000">IGV</option><option value="9997">EXO</option><option value="9998">INA</option></select>'+'<select name="afectacionigv[]" class="" style="display:none;"> <option value="10">10-GOO</option><option value="20">20-EOO</option><option value="30">30-FRE</option></select></td>'+
        ...
        '<td><input type="text" class="" name="precio_unitario[]" id="precio_unitario[]" value="'+precio_factura+'" onBlur="modificarSubototales(1)" size="7" onkeypress="return NumCheck2(event, this)" OnFocus="focusTest(this); return false;"  ></td>'+
...
// });
...

  {
    ...
    if ($("#codigo_tributo_18_3").val()=='9997')
        ...
        '<td><input type="text"  class="" name="descuento[]" id="descuento[]"  onBlur="modificarSubototales(1)" size="2" onkeypress="return NumCheck(event, this)" >'+
        '<span name="SumDCTO" id="SumDCTO'+cont+'" style="display:none"></span> <input type="hidden"  required="true" class="" name="sumadcto[]" id="sumadcto[]" >  </td>'+
        '<td hidden><input type="hidden" name="codigo_proveedor[]" id="codigo_proveedor[]" value="'+data.codigo_proveedor+'">'+data.codigo_proveedor+'</td>'+
        '<td hidden><input type="text" name="codigo[]" id="codigo[]" value="'+data.codigo+'" class="" style="display:none;" ></td>'+
        '<td><input type="hidden" name="unidad_medida[]" id="unidad_medida[]" value="'+data.abre+'">'+data.abre+'</td>'+
        '<td><input type="text" class="" name="precio_unitario[]" id="precio_unitario[]" value="'+data.precio_venta+'" onBlur="modificarSubototales(1)" size="7" onkeypress="return NumCheck2(event, this)" OnFocus="focusTest(this); return false;"  ></td>'+
        '<td><input type="text" class="" name="valor_unitario[]" id="valor_unitario[]" size="5"  value="'+precioOculto+'"    '+ exo +' onBlur="modificarSubototales(1"></td>'+
        ...
        '<input  type="hidden" name="mticbperu[]" id="mticbperu["'+cont+'"]" value="'+data.mticbperuSunat+'">'+
...
function enviarcorreo(idboleta) {
  ...
  Swal.fire({
      ...
      if (result.value) {
          $.post("../ajax/boleta.php?op=enviarcorreo&idbol=" + idboleta + "&ema=" + result.value, function(e) {
              Swal.fire({
                  title: e,
                  icon: 'success'
              });
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.post('../ajax/boleta.php?op=downFtp', { idboleta: idboleta }, function(e) {
        ...
        Swal.fire({
          icon: 'success',
          html: '<a href="' + data.cabext + '" download="' + data.cab + '">ARCHIVO JSON: ' + data.cab + '</a>',
          confirmButtonText: 'Aceptar'
        });
      });
    }
...
function agregarItemdetalle() {
    ...
    if ($("#nombre_tributo_4_p").val()=='9997')
        ...
        '<td><input type="text"  class="" required="true" name="cantidad_item_12[]" id="cantidad_item_12[]"  size="6" onkeypress="return NumCheck(event, this)"  value="'+$cantidadreal+'" >'+
        ...
        '<td><input type="hidden" name="unidad_medida[]" id="unidad_medida[]" value="'+$unidad_medida+'">'+$unidad_medida+'</td>'+
        '<td><input type="text" class="" name="precio_unitario[]" id="precio_unitario[]" value="'+$precio_boleta+'"  size="7" onkeypress="return NumCheck2(event, this)" OnFocus="focusTest(this); return false;"  ></td>'+
        '<td><input type="text" class="" name="valor_unitario[]" id="valor_unitario[]" size="5"  value="'+precioOculto+'"    '+ exo +' ></td>'+
  ...
  $.post('../ajax/boleta.php?op=mostrarxml', { idboleta }, function(response) {
    ...
    if (data.rutafirma) {
      ...
      $.get(rutaArchivo, function(xml) {
        ...
        $('#modalxml').contents().find('body').get(0).textContent = formattedXml;
      ...
      if (result.value) {
        $.post("../ajax/boleta.php?op=generarxml", {idboleta : idboleta}, function(e) {
          ...
          Swal.fire({
            ...
            html: 'Se ha generado el archivo XML: <a href="' + data.cabextxml + '" download="' + data.cabxml + '">' + data.cabxml + '</a> Haz clic en el nombre para descargarlo.',
            icon: 'success'
          });
          tabla.ajax.reload();
        });
...
function montotarjetadc(idboleta)
{
    Swal.fire({
        ...
        if (result.value) {
          $.post("../ajax/boleta.php?op=montotarjetadc_&monto=" + result.value, {
            idboleta: idboleta
          }, function(e) {
            Swal.fire({
              title: e,
...
function cambiartransferencia(idboleta) {
    Swal.fire({
      ...
      if (result.value) {
        $.post("../ajax/boleta.php?op=cambiartransferencia&opcion=" + result.value, {
          idboleta: idboleta
        }, function(e) {
          Swal.fire({
            title: e,
...
function cargarTiposDocIde() {
  $.ajax({
      url: 'https://wfacx.com/sistema/ajax/catalogo6.php?action=listar2',
      type: 'GET',
      dataType: 'json',
      success: function(data) {
