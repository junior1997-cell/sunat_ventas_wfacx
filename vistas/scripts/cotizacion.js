...
function cancelarform() {
    Swal.fire({
        title: 'Â¿Desea cancelar el comprobante?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            // Acciones a realizar si el usuario confirma la cancelaciÃ³n
            limpiar();
            evaluar2();
            detalles = 0;
            cont = 0;
...
function anular(idfactura) {
    bootbox.confirm("Â¿EstÃ¡ Seguro de anular la factura?", function (result) {
        if (result) {
...
//FunciÃ³n para enviar respuestas por correo 
function enviarcorreo(idfactura) {
    swal.fire({
        title: "Â¿EstÃ¡ seguro de enviar correo al cliente?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "SÃ­",
        cancelButtonText: "No"
...
function enviarxmlSUNAT(idfactura) {
    Swal.fire({
        title: 'Â¿EstÃ¡ seguro de enviar archivo firmado a SUNAT?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'SÃ­',
        cancelButtonText: 'No',
...
//FunciÃ³n para dar de baja registros
function downFtp(idcotizacion) {
    bootbox.confirm("Â¿EstÃ¡ Seguro de descargar los archivos?", function (result) {
        if (result) {
...
function baja(idcotizacion) {
    swal.fire({
        title: "Â¿Desea anular la cotizaciÃ³n?",
        icon: "warning",
        showCancelButton: true,
...
function NumCheck(e, field) {
    ...
    if (key > 47 && key < 58) {
        if ($(field).val() === "") return true; // Convertimos 'field' en objeto jQuery
        var existePto = (/[.]/).test($(field).val()); // Convertimos 'field' en objeto jQuery
        if (existePto === false) {
            regexp = /.[0-9]{10}$/;
        } else {
            regexp = /.[0-9]{2}$/;
        }
        return !(regexp.test($(field).val())); // Convertimos 'field' en objeto jQuery
    }

    if (key == 46) {
        if ($(field).val() === "") return false; // Convertimos 'field' en objeto jQuery
        regexp = /^[0-9]+$/;
        return regexp.test($(field).val()); // Convertimos 'field' en objeto jQuery
    }
    return false;
...
//FunciÃ³n para el formato de los montos 
...
function actualizanorden() {
    ...
    for (var i = 0; i < total.length; i++) {
        var contNO = i + 1;
        total[i].value = contNO;
        document.getElementsByName("numero_orden")[i].innerHTML = contNO;
        document.getElementsByName("numero_orden_item[]")[i].value = contNO;
    }
}
...
function nuevafactura(idcotizacion, mm) {
    swal.fire({
        title: '¡seguro de generar nueva factura?',
        icon: 'warning',
        showCancelButton: true,
