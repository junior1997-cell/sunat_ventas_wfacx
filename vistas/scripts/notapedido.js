...
function limpiar()
{
    ...
    document.getElementById("mensaje700").style.display='none';
     ...
     $("#tipo_doc_ide").val("0");
      //pARA CARGAR el id del cliente varios 08/01/2023
...
function guardaryeditarBoleta(e)
{
    ...
    if(sw!=0){
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Revizar précio!, cantidad o Stock',
            onClose: () => { inpP.focus() }
        })
    }else{
        Swal.fire({
            title: '¿Estás seguro?',
            text: "¿Desea emitir la nota de pedido?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
 ...
 //Función para aceptar solo numeros con dos decimales
    ...
    factorc)
  {
     ...
     if (idarticulo!="")
    {
        ...
        '<input type="text" class=""  name="subtotalBD[]" id="subtotalBD["'+cont+'"]" onBlur="modificarSubototales()" value="0.00"></td>'+
...
$('#basic-addon1').click(function() {
  ...
  for (var i = 0; i < ccuotas; i++) {
    ...
    $('#divmontocuotas').append('<input type="text" name="montocuota[]" value="">');
    
    var currentDate = new Date();
    currentDate.setMonth(currentDate.getMonth() + i);
    var formattedDate = currentDate.toISOString().split('T')[0];
    
    $('#divfechaspago').append('<input type="date" name="fechapago[]" value="' + formattedDate + '">');
  }
});
