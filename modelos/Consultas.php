...
select
 ...
 select
    ...
    {
        $sql = "select dia, sum(VentasDia) as VentasDia
    from 
    (
        select
    ...
    sum(
        case 
            when f.tipo_moneda_28='USD' then f.importe_total_venta_27 * f.tcambio
            else f.importe_total_venta_27
        end
    ) as ventasdia
from factura f 
where yearweek(f.fecha_emision_01, 1) = yearweek(curdate(), 1) 
and f.estado in('5','1','6','4') 
and f.idempresa = '$idempresa'  -- asumiendo que tienes la variable $idempresa
group by dayofweek(f.fecha_emision_01)
