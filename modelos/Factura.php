...
//**************************************************************************
 ...
 <!--<cac:TaxSubtotal>
  ...
  //     f.tipo_moneda_28 as  moneda,
  //     f.tcambio,
  //     (f.tcambio * importe_total_venta_27) as valordolsol,
  //     f.otroscargos,
  //     f.formapago
  //     from
  //     factura f inner join persona p on f.idcliente=p.idpersona
  ...
  where
      date(f.fecha_emision_01) = current_date and e.idempresa = '$idempresa'
  group by
      f.idfactura
  order by
      f.idfactura desc;
  ...
  public function ventacabecera($idfactura, $idempresa)
  {
    $sql = "select
        ...
        GROUP_CONCAT(lpad(cu.ncuota,3,'0'),'|',cu.montocuota,'|',date_format(cu.fechacuota, '%Y-%m-%d')) as cuotas
          ...
          inner join usuario u on f.idusuario=u.idusuario
          INNER JOIN cuotas cu ON cu.idcomprobante = f.idfactura
          where
          f.idfactura = '$idfactura' and e.idempresa = '$idempresa' and cu.tipocomprobante = '01'
